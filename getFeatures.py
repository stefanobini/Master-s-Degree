...
for i in select_nets:
    ...
    Net = N['Net'].todense()    # trasform matrix in dense matrix where most of values are not zero.
    print("Net %d, NNofile_keywords=%d \n" % (i, np.count_nonzero(Net)))
    Nets.append(minmax_scale(Net))  # rescale the values between [0, 1]
    input_dims.append(Net.shape[1])
    '''
    # Load direct dependency matrices
    N = sio.loadmat('./dir_matrices/' + org + '_dir_net_' + str(i) + '.mat', squeeze_me=True)
    Net = N['dir_matrix']
    Nets.append(Net)
    input_dims.append(Net.shape[1])
